    ...
    */
    ...
    var SBConversations = {
        ...
        getListCode: function (conversation, status) {
            ...
            return `<li data-user-id="${conversation['user_id']}" data-conversation-id="${conversation['conversation_id']}" data-conversation-status="${status}"${!SBF.null(conversation['conversation_source']) ? ` data-conversation-source="${conversation['conversation_source']}"` : ''}><div class="sb-profile"><img loading="lazy" src="${conversation['profile_image']}"><span class="sb-name${is_title ? ' sb-custom-name' : ''}">${is_title ? conversation.title : conversation['first_name'] + ' ' + conversation['last_name']}</span><span class="sb-time">${SBF.beautifyTime(conversation['creation_time'])}</span></div><p>${strip.strip(message)}</p></li>`;
        },
    ...
    $(document).ready(function () {
        ...
        $(header).on('click', '.sb-version', function () {
            ...
            SBF.ajax({
                ...
                let code = '';
                let names = { 'sb': 'Support Board', 'slack': 'Slack', 'dialogflow': 'Dialogflow', 'tickets': 'Tickets', 'woocommerce': 'Woocommerce', 'ump': 'Ultimate Membership Pro', 'perfex': 'Perfex', 'whmcs': 'WHMCS', 'aecommerce': 'Active eCommerce', 'messenger': 'Messenger', 'whatsapp': 'WhatsApp', 'armember': 'ARMember', 'telegram': 'Telegram', 'wechat': 'WeChat', 'twitter': 'Twitter', 'zendesk': 'Zendesk', 'gbm': 'Google Business Messages', 'martfury': 'Martfury' };
                let updates = false;
        ...
        */
        ...
        $(settings_area).on('click', '#whatsapp-twilio-btn .sb-btn, #sms-btn .sb-btn, #wechat-btn .sb-btn, #twitter-callback .sb-btn, #gbm-webhook .sb-btn', function (e) {
            ...
            dialog(`<pre>${SB_URL + (id == 'sms-btn' ? '/include/api.php' : ('/apps/' + (id == 'wechat-btn' ? 'wechat' : (id == 'twitter-callback' ? 'twitter' : (id == 'gbm-webhook' ? 'gbm' : 'whatsapp'))) + '/post.php')) + cloudURL().replace('&', '?')}</pre>`, 'info');
            return false;
            e.preventDefault();
        });
