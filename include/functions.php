...
$sb_apps = ['dialogflow', 'slack', 'wordpress', 'tickets', 'woocommerce', 'ump', 'perfex', 'whmcs', 'aecommerce', 'messenger', 'whatsapp', 'armember', 'telegram', 'wechat', 'twitter', 'zendesk', 'gbm', 'martfury'];
...
function sb_email_send($to, $subject, $body, $sender_suffix = '') {
    ...
    if (!$host && sb_is_cloud()) $settings = ['email-server-host' => CLOUD_SMTP_HOST, 'email-server-user' => CLOUD_SMTP_USERNAME, 'email-server-password' => CLOUD_SMTP_PASSWORD, 'email-server-from' => CLOUD_SMTP_SENDER, 'email-sender-name' => CLOUD_SMTP_SENDER_NAME, 'email-server-port' => CLOUD_SMTP_PORT];
                                ...
                                if (strpos($message, 'quoted-printable')) {
                                    ...
                                    if ($encoding) {
                                        switch ($encoding) {
                                            ...
                                            default:
                                                $message = quoted_printable_decode($message);
                                                if (strpos($message, ' =E')) $message = mb_convert_encoding($message, 'UTF-8', mb_detect_encoding($message, 'ISO-8859-1, ISO-8859-2'));
                                                break;
                                        }
                                    }
                                }
